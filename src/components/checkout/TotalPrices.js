...
const TotalPrices = ({subscriptions, services}) => {
    const cart = getCart();
        ...
        <Box sx={{my: 2}}>
            ...
            <Box sx={{p: 1}}>
                <Stack direction="row" justifyContent="space-between">
                    ...
                    <Typography variant="h6" sx={{fontWeight: 700}} children={`$${Number(subtotal).toFixed(2)}`} />
                </Stack>
                <Stack direction="row" justifyContent="space-between">
                    <Typography variant="h6" sx={{fontWeight: 700}} children="GST (INCLUSIVE)" />
                    <Typography variant="h6" sx={{fontWeight: 700}} children={`$${Number(gst).toFixed(2)}`} />
                </Stack>
                <Stack direction="row" justifyContent="space-between">
                    <Typography variant="h6" sx={{fontWeight: 700}} children="TOTAL" />
                    <Typography variant="h6" sx={{fontWeight: 700}} children={`$${Number(total).toFixed(2)}`} />
                </Stack>
